...
const createTables = async () => {
  ...
  try {
    ...
    // 12b. Tambahkan kolom ml_details jika belum ada (untuk database yang sudah ada)
    try {
      await connection.query(`
        ALTER TABLE applications ADD COLUMN IF NOT EXISTS ml_details JSON
      `);
    } catch (alterErr) {
      // Kolom mungkin sudah ada atau DB tidak support IF NOT EXISTS — abaikan
      console.log("   ℹ️  ml_details column: already exists or skipped");
    }

    // 12c. Ubah match_score ke FLOAT jika masih INT
    try {
      await connection.query(`
        ALTER TABLE applications MODIFY COLUMN match_score FLOAT DEFAULT 0
      `);
    } catch (alterErr) {
      console.log("   ℹ️  match_score column type: already float or skipped");
    }
